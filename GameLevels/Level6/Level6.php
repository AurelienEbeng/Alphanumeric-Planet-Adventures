...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['refresh'])) {
        ...
        // check if the input contains 6 numbers
        if (count($input) === 2) {
            $smallest = min($input);
            $largest = max($input);
            $generatedNumbers = $_SESSION['numbers'];
            sort($generatedNumbers);
            $correctSmallest = $generatedNumbers[0];
            $correctLargest = $generatedNumbers[5];
            if ($smallest == $correctSmallest && $largest == $correctLargest) {
                $_SESSION["hasWon"] = true;
                header("Location: ../Msn/Win.php");
                exit();
            } else {
            ...
            $message = "Please enter exactly 2 numbers.";
        }
    }
}
?>
...
<body>
    ...
    <div class="login">        
        <form action="" method="post" class="login__form" id="loginForm">
            <div><h1>Number Game Level 6</h1></div>  
            <p style="color:white">Identify the smallest and the largest number:</p>
            <div>
                <br>
                <label for="numbers" style="color:white">Random Numbers:</label>
                <input type="text" id="numbers" name="numbers" value="<?= implode(" ", $_SESSION['numbers']) ?>" readonly>
            </div>
            <div>
                <label for="answer" style="color:white">Enter 2 numbers (space-separated):</label>
